...
class BluetoothScaleService {
  ...
  Future<void> readWeight() async {
    ...
    try {
      if (_weightCharacteristic!.properties.read) {
        ...
        print('⚠️ [ScaleService] Characteristic does not support READ (NOTIFY-only scale)');
        print('💡 [ScaleService] Ensure weight is ON the scale and try changing it slightly');
        
        // Check if notify is enabled
        bool isNotifying = await _weightCharacteristic!.isNotifying;
        print('🔔 [ScaleService] Notifications currently enabled: $isNotifying');
  ...
  Future<void> _discoverServices(BluetoothDevice device) async {
    ...
    for (var service in services) {
      ...
      for (var characteristic in service.characteristics) {
        ...
        if (uuid.contains('2A05') ||  // Service Changed
            ...
            uuid.contains('2A04')) {  // Peripheral Preferred Connection Parameters
    ...
    if (candidates.isNotEmpty) {
      print('🎯 [ScaleService] Using first candidate: ${candidates.first.uuid}');
  ...
  Future<void> _setupNotification(BluetoothCharacteristic characteristic) async {
    print('🔔 [ScaleService] Setting up notification for ${characteristic.uuid}');
    ...
    try {
      ...
      characteristic.lastValueStream.listen((value) {
        print('📦 [ScaleService] Received data: $value (length: ${value.length})');
        _parseWeightData(value);
      });
    } catch (e) {
